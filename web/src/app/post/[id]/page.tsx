...
import type { ToastState } from "@/types/library";
...
export default function PostSchedulePage() {
  ...
  const assetId = useMemo(() => String(params?.id || ""), [params]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const fetched = await libraryService.getLibraryItem(numericId);
        if (fetched) {
          setItem(fetched);
    ...
    if (err && typeof err === "object") {
      ...
      if (data) {
        ...
        if (Array.isArray(detail) && detail.length) {
          ...
          const msg = first && typeof first["msg"] === "string" ? (first["msg"] as string) : undefined;
          if (msg) return msg;
        }
  ...
  const handleSubmit = async (immediate: boolean) => {
    ...
    try {
      ...
      showToast(
        immediate ? "Post published successfully!" : "Post scheduled successfully!",
        "success"
      );
  ...
  // Instagram is the only supported platform currently.
        ...
        <Card className="bg-[#D9D9D9]/[0.72]">
          ...
          <CardContent className="space-y-6">
            {item && (
              <div className="flex gap-4 p-4 border rounded-lg bg-muted/50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold mb-1 line-clamp-2">{item.title}</p>
            ...
            <div className="space-y-2">
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-sm border bg-white">
                <span className="font-medium">Instagram</span>
                <span className="text-xs text-muted-foreground">(only supported)</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">Leave empty to post immediately using Post Now.</p>
            </div>

            <div className="flex gap-3 pt-2">
              <Button onClick={() => handleSubmit(false)} disabled={isLoading} className="flex-1">
                Schedule Post
              </Button>
